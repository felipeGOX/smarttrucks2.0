...
@section('content')
                    ...
                    <tbody>
                        ...
                        @foreach ($establecimientos as $establecimiento)
                            <tr>
                                ...
                                <td>{{ $establecimiento->ruta }}</td>
                                <td>{{ $establecimiento->distrito }}</td>
                                <td>{{ $establecimiento->red }}</td>
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        @can('ver-establecimiento')
                                            <a href="{{ route('establecimientos.show', $establecimiento->id) }}" class="btn btn-info"><i
                                                    class="fas fa-eye"></i></a>
                                        @endcan
                                        @can('editar-establecimiento')
                                            <a href="{{ route('establecimientos.edit', $establecimiento->id) }}" class="btn btn-primary"><i
    ...
    <Script type="text/javascript">
        $('#limit').on('change', function() {
            window.location.href = "{{ route('establecimientos.index') }}?limit=" + $(this).val() + '&search=' + $(
                '#search').val()
        })

        $('#search').on('keyup', function(e) {
            if (e.keyCode == 13) {
                window.location.href = "{{ route('establecimientos.index') }}?limit=" + $('#limit').val() + '&search=' +
                    $(this).val()
            }
        })
    </Script>
