...
@section('content')
    ...
    <Script type="text/javascript">
        $('#limit').on('change', function() {
            window.location.href = "{{ route('recepciones.index') }}?limit=" + $(this).val() + '&search=' + $(
                '#search').val()
        })

        $('#search').on('keyup', function(e) {
            if (e.keyCode == 13) {
                window.location.href = "{{ route('recepciones.index') }}?limit=" + $('#limit').val() + '&search=' +
                    $(this).val()
            }
        })
    </Script>
