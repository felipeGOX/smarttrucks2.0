...
@section('content')
    <div class="card mt-3">
        ...
        <div class="card-body">
            <div class="row">
                ...
                @if ($equiposRecorridos->total() > 10)
                    {{ $equiposRecorridos->links() }}
                @endif
            </div>
                    ...
                    <tbody>
                        ...
                        @foreach ($equiposRecorridos as $equipoRecorrido)
                            <tr>
                                ...
                                <td>{{ $equipoRecorrido->empleado }}</td>
                                <td>{{ $equipoRecorrido->camion }}</td>
                                <td>
                                    <div class="btn-group" role="group" aria-label="Basic example">
                                        @can('ver-equipoRecorrido')
                                            <a href="{{ route('equiposRecorridos.show', $equipoRecorrido->id) }}" class="btn btn-info"><i
                                                    class="fas fa-eye"></i></a>
                                        @endcan
                                        @can('editar-equipoRecorrido')
                                            <a href="{{ route('equiposRecorridos.edit', $equipoRecorrido->id) }}" class="btn btn-primary"><i
    ...
    <Script type="text/javascript">
        $('#limit').on('change', function() {
            window.location.href = "{{ route('equiposRecorridos.index') }}?limit=" + $(this).val() + '&search=' + $(
                '#search').val()
        })

        $('#search').on('keyup', function(e) {
            if (e.keyCode == 13) {
                window.location.href = "{{ route('equiposRecorridos.index') }}?limit=" + $('#limit').val() + '&search=' +
                    $(this).val()
            }
        })
    </Script>
